...
def _coerce_for_masked_scatter(destination: torch.Tensor, mask: torch.Tensor, source: torch.Tensor):
    ...
    if mask.device != destination.device:
        ...
        if sam_model is not None and (input_ids.shape[1] != 1 or self.training) and torch.sum(images[0][1]).item() != 0:
            ...
            for image, crop_shape in zip(images, images_spatial_crop):
                ...
                if images_in_this_batch:
                    ...
                    scatter_mask, scatter_source = _coerce_for_masked_scatter(
                        inputs_embeds[idx],
                        images_seq_mask[idx].unsqueeze(-1),
                        images_in_this_batch,
                    )
                    inputs_embeds[idx].masked_scatter_(scatter_mask, scatter_source)
...
class DeepseekOCR2ForCausalLM(DeepseekV2ForCausalLM):
    ...
    def infer(self, tokenizer, prompt='', image_file='', output_path = '', base_size=1024, image_size=768, crop_mode=True, test_compress=False, save_results=False, eval_mode=False):
        ...
        for text_sep, image in zip(text_splits, images):
            ...
            if crop_mode:
                ...
                if image_size == 768:
                    ...
                    valid_img_tokens += len(images_crop_list) * 256
        ...
        eos_token_id = tokenizer.eos_token_id if tokenizer.eos_token_id is not None else self.config.eos_token_id
        ...
        if '<image>' in conversation[0]['content'] and save_results:
            ...
            with open(f'{output_path}/result.raw.txt', 'w', encoding='utf-8') as afile:
                afile.write(outputs)
